...
if st.button("Run Compliance Scan"):
    try:
        ...
        pdf.cell(0, 10, txt="Aura AI Compliance Certificate", ln=True, align='C')
        pdf.ln(10)
        pdf.set_font("helvetica", size=12)
        pdf.cell(0, 10, txt=f"Official Audit for Model ID: {model_id}", ln=True)
        pdf.cell(0, 10, txt=f"Compliance License: {lic}", ln=True)
        pdf.cell(0, 10, txt="Audit Signature: Aura Autonomous Governance System", ln=True)
        
        # FIX: fpdf2 output('S') returns bytes directly, no .encode() needed
        pdf_output = pdf.output()
        
        st.download_button(
            label="📥 Download Compliance PDF", 
            data=bytes(pdf_output), 
            file_name=f"Aura_Audit_{model_id.replace('/', '_')}.pdf", 
